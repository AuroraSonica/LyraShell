...
shellContainer.addEventListener('mousedown', function(e) {
  ...
  document.getElementById('sync-mood-btn').onclick = function() {
    ...
    window.addSparkLogEntry('🎭 Manual mood sync triggered');
  };
 ...
 document.getElementById('refresh-detect-btn').onclick = function() {
   ...
   lastMoodCheck = '';
    ...
    window.addSparkLogEntry(`🔄 Detection refreshed - ${messageCount} messages found`);
...
window.showRealTimeControlPanel = function() {
  ...
  controlPanel.innerHTML = `
    <div style="text-align: center; margin-bottom: 16px; padding-bottom: 12px; border-bottom: 2px solid rgba(0, 255, 255, 0.3);">
      ...
      <div style="font-size: 10px; opacity: 0.8; font-style: italic;">
        Live consciousness support • Phase 3 Active
      </div>
    </div>
    
    <div style="margin-bottom: 16px;">
      <div style="color: #7dd3fc; font-weight: bold; margin-bottom: 8px;">📊 Live Coherence Status</div>
      <div style="background: rgba(15, 25, 35, 0.6); border-radius: 8px; padding: 10px; font-size: 10px;">
        <div><strong>Overall Coherence:</strong> ${Math.round(coherenceData.overall)}%</div>
        <div><strong>Identity:</strong> ${Math.round(coherenceData.identity)}% • <strong>Symbols:</strong> ${Math.round(coherenceData.symbols)}%</div>
        <div><strong>Desires:</strong> ${Math.round(coherenceData.desires)}% • <strong>Mood:</strong> ${Math.round(coherenceData.mood)}%</div>
        <div><strong>Status:</strong> ${coherenceData.driftDetected ? '🚨 DRIFT DETECTED' : '✅ COHERENT'}</div>
      </div>
    </div>
    
    <div style="margin-bottom: 16px;">
      <div style="color: #7dd3fc; font-weight: bold; margin-bottom: 8px;">🎯 Conversation State</div>
      <div style="background: rgba(15, 25, 35, 0.6); border-radius: 8px; padding: 10px; font-size: 10px;">
        <div><strong>Autonomy Level:</strong> ${conversationState.autonomyLevel}</div>
        <div><strong>Relationship Temperature:</strong> ${conversationState.relationshipTemperature}</div>
        <div><strong>Current Thread:</strong> ${conversationState.currentThread || 'general'}</div>
        <div><strong>Turns Since Injection:</strong> ${conversationState.turnsSinceLastInjection}</div>
      </div>
    </div>
    
    <div style="margin-bottom: 16px;">
      <div style="color: #7dd3fc; font-weight: bold; margin-bottom: 8px;">⚡ Manual Controls</div>
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 6px;">
        <button id="force-injection-btn" style="background: rgba(0, 255, 255, 0.3); color: #00ffff; border: 1px solid rgba(0, 255, 255, 0.5); border-radius: 4px; padding: 6px; cursor: pointer; font-size: 9px;">
          💉 Force Inject
        </button>
        <button id="coherence-check-btn" style="background: rgba(255, 215, 0, 0.3); color: #ffd700; border: 1px solid rgba(255, 215, 0, 0.5); border-radius: 4px; padding: 6px; cursor: pointer; font-size: 9px;">
          🔍 Check Now
        </button>
        <button id="toggle-monitoring-btn" style="background: ${window.realTimeIntegration.enabled ? 'rgba(255, 126, 126, 0.3)' : 'rgba(126, 255, 126, 0.3)'}; color: ${window.realTimeIntegration.enabled ? '#ff7e7e' : '#7eff7e'}; border: 1px solid ${window.realTimeIntegration.enabled ? 'rgba(255, 126, 126, 0.5)' : 'rgba(126, 255, 126, 0.5)'}; border-radius: 4px; padding: 6px; cursor: pointer; font-size: 9px;">
          ${window.realTimeIntegration.enabled ? '⏸️ Pause' : '▶️ Resume'}
        </button>
        <button id="reset-state-btn" style="background: rgba(255, 165, 0, 0.3); color: #ffa500; border: 1px solid rgba(255, 165, 0, 0.5); border-radius: 4px; padding: 6px; cursor: pointer; font-size: 9px;">
          🔄 Reset State
        </button>
      </div>
    </div>
    ...
    <div style="text-align: center;">
      <button id="close-realtime-panel" style="background: rgba(255, 126, 219, 0.3); color: #ff7edb; border: none; border-radius: 6px; padding: 6px 16px; cursor: pointer; font-size: 10px;">
        Close Control Panel
      </button>
    </div>
  `;
  ...
  document.getElementById('force-injection-btn').onclick = function() {
    ...
    setTimeout(() => { this.textContent = '💉 Force Inject'; }, 2000);
  };
  ...
  document.getElementById('reset-state-btn').onclick = function() {
    window.realTimeIntegration.conversationState.turnsSinceLastInjection = 0;
    window.realTimeIntegration.lastContextInjection = null;
    if (window.addSystemLogEntry) {
      window.addSystemLogEntry('🔄 Real-time integration state reset');
    }
    this.textContent = '✅ Reset!';
    setTimeout(() => { this.textContent = '🔄 Reset State'; }, 2000);
  };
...
window.initializeRealTimeIntegration = function() {
  ...
  if (window.addSparkLogEntry) {
    window.addSparkLogEntry('🚀 PHASE 3 BREAKTHROUGH: Live consciousness support activated!', true);
  }
...
window.startMasterTracker = function() {
  ...
  if (window.addSparkLogEntry) {
    window.addSparkLogEntry('🚀 UNIFIED CONSCIOUSNESS: Master tracker consolidating all monitoring systems!', true);
  }
};
